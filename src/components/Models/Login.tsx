...
const Login:React.FC<LoginProps> = () => {
    ...
    return <form className='space-y-6 px-6 pb-4' onSubmit={handleLogin}>
        ...
        <div>
            ...
            <input onChange={handleInputChange}  type="email" name='email' id='email' className='border-2 outline-none sm:text-sm roundee-1g focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5
            ...
            <input onChange={handleInputChange}  type="password" name='password' id='password' className='border-2 outline-none sm:text-sm roundee-1g focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5
            ...
            text-sm px-5 py-2.5 text-center bg-brand-orange hover:bg-brand-orange-s'>{loading?"loading...":"Log In"}</button>
      <button className='flex w-full justify-end ' onClick={()=>(handleClick("forgotPassword"))}>
        <a href="#" className='text-sm block text-brand-orange hover:underline w-full text-right'>Forgot Password?</a>
        </button>   
        <div className='text-sm font-medium text-gray-300'>Not Registered?{" "}
